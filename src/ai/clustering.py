...
@click.command()
@click.option("--path", default="data/CIC/flows_labeled/Thursday-WorkingHours-Afternoon-Infilteration.pcap_ISCX.csv")
@click.option("--model_type", type=click.Choice(['random_forest', 'knn']), default='knn')
...
def analyze_predictions(df: pd.DataFrame, model, x_pca):
    ...
    label_mapping = {i: label for i, label in enumerate(df['Label'].astype('category').cat.categories)}
...
def plot_clusters(x_pca, predictions, model_name):
    ...
    scatter = plt.scatter(x_pca_reduced[:, 0], x_pca_reduced[:, 1], c=numeric_predictions, cmap='viridis', s=10, alpha=0.7)
