...
class Feeder(ComponentServicer):
    ...
    def forward(self, msg: ComponentMessage, context):
        ...
        flow_csv = self.argus_to_flow_csv(argus, self.file_name.replace(".pcap", "_flows.csv"))
    ...
    def pcap_to_argus(self, pcap_file: str) -> str:
        ...
        assert subprocess.run(cmd, check=True).returncode == 0, f"\n!!!Argus failed to process {pcap_file}!!!\n"
    ...
    def argus_to_flow_csv(self, argus_file: str, output_csv: str) -> str:
        cmd = [
            ...
            "-r", argus_file,
            "-c", ",",
            "-s", "stime,flgs,proto,saddr,daddr,sport,dport,bytes,pkts,state",
        ]
        assert subprocess.run(cmd, stdout=open(output_csv, "w"), check=True), f"\n!!!Failed to convert {argus_file} to CSV!!!\n"
    ...
    def preprocess_flow_row(self, flow_row):
        ...
        if numeric_row.shape[0] < 80:
            numeric_row = np.pad(numeric_row, (0, 80 - numeric_row.shape[0]), 'constant')
    ...
    def flow_to_model(self, flow_csv, model_host='localhost', model_port=50052) -> None:
